...
public class UserDaoImpl implements UserDao {
    ...
    @Override
    public List<DBObject> withinMultiPolygon(String collection, String locationField, List<double[]> polygon1, List<double[]> polygon2, DBObject fields, DBObject query, int limit) {
        if(query==null)
            query = new BasicDBObject();

        List<List<double[]>> polygons1 = new LinkedList<>();
        polygons1.add(polygon1);

        List<List<double[]>> polygons2 = new LinkedList<>();
        polygons2.add(polygon2);

        List< List<List<double[]>>> lists=new LinkedList<>();
        lists.add(polygons1);
        lists.add(polygons2);
